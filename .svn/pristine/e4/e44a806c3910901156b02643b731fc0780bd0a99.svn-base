...
// new code process start  by hari
//highlighter added by hari for highlight options 
...
//get colunm name by index method by hari
...
//highlighter function call for highlighiting Text
...
// additional process by hari

function InitiateProperty(GridID,PagerName) {
    jQuery("#" + GridID).jqGrid('navGrid', '#' + GridID + '_toppager', {
        add: false,
        edit: false,
        del: false,
        searchtext: "Search",
        ...
        cloneToTop: true
            ...
            { multipleSearch: true, closeAfterSearch: true, overlay: false, width: 460, showQuery: false });

    //jQuery("#" + GridID).jqGrid('navButtonAdd', '#' + GridID + '_toppager', {
    ...
    //    },
    //    position: "first",
    //    title: "Reorder Column For Working Process",
    //    cursor: "pointer"
    //});
   ...
   // jQuery("#" + GridID)[0].toggleToolbar();
    ...
    $("#" + GridID + "_toppager_center").hide();
    $("#" + GridID + "_toppager_right").hide();
    $("#" + GridID + "_toppager_left").attr("colspan", "5");
    $("#pg_" + GridID + "_toppager").attr('align', 'right');
    $("#pg_" + PagerName).attr('align', 'center');

   // $("#" + PagerName + "_center").css("width", "100%");
...
// color setting for Cell
function SetCellColor(cellvalue,BackgroundColor, TextColor)
{
return '<span class="cellWithoutBackground" style="background-color:' + BackgroundColor + '; color:white !important;">' + cellvalue;
}

function setRowColor(GridName,bgColor,TextColor)
{

    for (var i = 0; i < rowsToColor.length; i++) {
        $("#" + GridName).jqGrid('setRowData', rowsToColor[i], false, { color: TextColor, weightfont: 'bold', background: bgColor });
        $("#" + rowsToColor[i]).removeClass("myAltRowClass");
    }
}

function setRowColor_class(GridName, ClassName) {

    for (var i = 0; i < rowsToColor.length; i++) {
        $("#" + rowsToColor[i]).removeClass("myAltRowClass");
        $("#" + rowsToColor[i]).addClass(ClassName);
    }
}
