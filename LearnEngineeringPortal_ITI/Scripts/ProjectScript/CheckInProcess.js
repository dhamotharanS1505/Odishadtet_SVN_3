...
$(function () {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: Appendurl+"/Home/CheckAlreadyLogined",
            type: 'post',
            async: false,
            success: function (Rdata) {
                //alert(Rdata.Valid);
        ...
        $('.backtologin, #forget_backtoLogin, #ResetBacktoLogin, #OTPBacktoLogin').click(function (e) {
            e.preventDefault();
            $('#modelLogin').show();
            clearInputValues('modelLogin');
        ...
        $('.backtologin_New').click(function (e) {
            e.preventDefault();
            $('#modelLoginNew').show();
        ...
        $(document).on('click', '#btnForgetPassword', function (e) {
            ...
            if ($("#frmForgetPassword").valid()) {
                ...
                $.ajax({
                    ...
                    url: Appendurl+"/Home/ForgetPassword?MobileNumber=" + $('#mobileNo').val(),
                    type: 'Post',
        ...
        $('#btnResetPassword').click(function (e) {
            ...
            if ($("#frmResetPassword").valid()) {
                ...
                $.ajax({
                    ...
                    crossOrigin: true,
                    url: Appendurl+"/Home/GetResetPasswords?mobileNo=" + $('#frmResetPassword input[name="mobileNo"]').val() + "&otpCode=" + $("#otpCode").val() + "&newPassword=" + $("#newPassword").val(),
                    type: 'Post',
                    success: function (Rdata) {
                        var userId = Rdata.userID;
                        if (userId > 0) {
        ...
        $('#newuser').click(function () {
            window.location.href = "/Home/StudentRegistration";
        });
          ...
          //  alert("hi");
            ...
            if ($('#frmLogin').valid()) {
                ...
                $.ajax({
                    ...
                    url: Appendurl +"/Home/CheckLogininProcess",
                    data: Mydata,
                    type: 'Post',
                    success: function (Rdata) {
                        //alert("Rdata.role_type_code" + Rdata.role_type_code);
                        ...
                        if (( Rdata.role_type_code > 2) && Rdata.message == "")
                        {
                          ...
                          //  window.location.href = "/AdminProcess/AdminDashboard";
                            window.location.href = "/AdminDashBoard/DashBoardMain";

                        }
                        //else if (Rdata.role_type_code == 4 && Rdata.message == "" ) {
                        ...
                        //    //window.location.reload();
                        //    //  window.location.href = "/AdminProcess/AdminDashboard";
                        //    window.location.href = "/CollegeGroupAdmin/DashBoardMain";
                        //}                        
                            ...
                            $this.text('Login');
                          //  window.location.reload();
                            window.location.href = "/UserDashBoard/UserUsageDashboard";
                        }
                        else {
        ...
        $('#btnactivateaccount').click(function (e) {
            ...
            if ($("#mobileNootpverify").val().trim() == "") {
                ...
                $.ajax({
                    ...
                    crossOrigin: true,
                    url: Appendurl +"/Home/checkOTPisValid?userid=" + userId + "&OTP=" + $("#mobileNootpverify").val(),
                    type: 'Post',
                    success: function (Rdata) {
                        //  var userId = Rdata;
                        if (Rdata == 1) {
        ...
        //User Activation from special offer pacakege page
        $('#btnactivateaccount_new').click(function (e) {
            e.preventDefault();
            var $this = $(this);
            var userId = $('#sessionUserId').val();
            if ($("#mobileNootpverifySplOffer").val().trim() == "") {
                ShowAlert("OTP Validation", "Please Enter OTP to Activate", "error");
            }
            else {
                $.ajax({
                    ...
                    crossOrigin: true,
                    url: Appendurl +"/Home/checkOTPisValid?userid=" + userId + "&OTP=" + $("#mobileNootpverifySplOffer").val(),
                    type: 'Post',
                    success: function (Rdata) {
                        if (Rdata == 1) {
                            $('.modelErrorTxt').text(Rdata.message);
                            $('.modelErrorTxt').show();
                            $('#modelAccountActivation, #modelactivateuser_new').hide();
                            clearInputValues('modelactivateuser_new');
                            $('#modelactivateuser_new').hide();
                            $('#modelLoginNew').show();
                            $('#sessionUserId').val('');
        ...
        $("#frmLoginNew #loginIDnew").blur(function () {
            var value = $("#frmLoginNew #loginIDnew").val();
            var intRegex = /[0-9 -()+]+$/;
            if (intRegex.test(value)) {
                phoneNumber = true;
            }
        ...
        $("#frmLoginNew #loginIDnew").keyup(function () {
            var value = $("#frmLoginNew #loginIDnew").val();
            var intRegex = /[0-9 -()+]+$/;
            if (intRegex.test(value)) {
                phoneNumber = true;
        ...
        $('#btnforgetpassword').click(function (e) {
            ...
            if ($('#txtfogetpasswrd').val() != '' && $('#txtfogetpasswrd').val() != null) {
                $.ajax({
                    type: "post",
                    url: Appendurl +"/Home/ForgetPassword?MobileNumber=" + $('#txtfogetpasswrd').val(),
                    datatype: "json",
                    beforeSend: function () {
                        $('#btnforgetpassword').parent().find('img:eq(0)').show();
                    },
                    success: function (rdata) {
                        if (rdata.userID > 0) {
                    ...
                    complete: function () {
                        $('#btnforgetpassword').parent().find('img:eq(0)').hide();
                ...
                ShowAlert("Forget password", "Please enter your mobile number", "warning");
            }
        });
        $(document).on('click', '#btnLoginNew', function (e) {
            e.preventDefault();

            $('.modelErrorTxt').text('');
            $('.modelErrorTxt').hide();
            $("#frmLoginNew").validate({
                rules: {
                    loginIDnew: {
                        required: true,
                        //  email: function () { if (phoneNumber == false) { return true; } },
                ...
                messages: {
                    loginIDnew: { required: "Enter your Email or Mobile number" },
                    Passwordnew: { required: "Enter the password" }
                }
            });
            if ($('#frmLoginNew').valid()) {

                alert("hi");

                var $this = $(this);
                var Mydata = {
                    FirstName: $('#loginIDnew').val(),
                ...
                $.ajax({
                    ...
                    type: 'Post',
                    success: function (Rdata) {
                        //if (Rdata.role_type_code == 3) {
                        //        var win = window.open('http://admin.learnengg.com', '_blank');
                        //        win.focus();
                        //        $this.text('Login');
                        //}
                        //else {
                        ...
                        if (Rdata.role_type_code == 4 && Rdata.message == "" || Rdata.role_type_code == 5 && Rdata.message == "" || Rdata.role_type_code == 3 && Rdata.message == "" || Rdata.role_type_code == 6 && Rdata.message == "") {
                            $('.modelErrorTxt').hide();
                            $this.text('Redirecting..');
                            $("#sessionUserId").val(Rdata.userId);
                            $("#loginUserName").text("Hi " + Rdata.FirstName);
