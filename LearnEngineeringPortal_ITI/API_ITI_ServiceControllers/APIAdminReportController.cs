...
namespace Odishadtet.APIServiceControllers
{
    public class APIAdminReportController : ApiController
    {
        ...
        [HttpGet]
        public List<UserReadHistoryModel> UniversityReadHistorySummarySpilit(int DepartmentID, int collegeID, int SubjectID, int SemsterID, string DateFrom, string DateTo, int SubjectType, int CollegeGrpId = 0)
        {
            List<UserReadHistoryModel> univreadhistry = new List<UserReadHistoryModel>();

            univreadhistry = _AdminReportRepository.UniversityReadHistorySummarySpilit(DepartmentID, collegeID, SubjectID, SemsterID, DateFrom, DateTo, SubjectType, CollegeGrpId);
