    ...
    public class college_masterController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "college_id,college_code,college_name,total_license,college_mobile,college_email,password")] college_master college_master)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    college_group_id = cgm.college_group_id,
                ...
                db.Entry<college_master>(college_master).State = EntityState.Added;
                db.Entry<college_group_master>(cgm).State = EntityState.Added;

                // map college group for 3dm group admin also
                string collegeGrpName = "3DM ClassRoom";
                collegeGrpName = ConfigurationManager.AppSettings["AdminGroup"];

                var collegegroupid_3dm = db.college_group_master.SingleOrDefault(x => x.college_group_name == collegeGrpName);

                if (collegegroupid_3dm != null)
                {
                    var cgmm_3dm = new college_group_map()
                    {
                        college_group_map_id = collegegroupid_3dm.college_group_id,
                        college_group_id = collegegroupid_3dm.college_group_id,
                        college_id = college_master.college_id,
                        enter_by = 1,
                        enter_on = DateTime.Now
                    };
                    db.college_group_map.Add(cgmm_3dm);
                }
