...
namespace Odishadtet.DAL
{
    ...
    public class StudentUserService : IStudentUserService
    {
        ...
        string PageName = "StudentService.cs";
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                ...
                try
                {
                    var excelData = (from sm in contextsdce.subject_master
                                     ...
                                     where dm.department_id == Department && dsm.semester == Semester
                                     select new
                                     {
                                         //um.university_name,
                                         dm.department_name,
                    ...
                    //                       }).Distinct().ToList();
                    return jsSerializer.Serialize(excelData);
                }
                catch (Exception ex)
                {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                ...
                try
                {
                    var excelData = (from sm in contextsdce.subject_master
                                     ...
                                     join dm in contextsdce.department_master on dsm.department_id equals dm.department_id
                                     //join um in contextsdce.university_master on dm.UniversityID equals um.univ_id
                                     orderby dsm.semester, sm.subject_name
                                     where dm.department_id == Department //&& dsm.semester == Semester
                                     select new
                                     {
                                         //um.university_name,
        ...
        public bool GetRandomCode(long userId, string code)
        {
            ...
            var userCodes = new StudentUserMasterAccess().GetAllRandomCodes();
            if (userCodes.Any(t => t.std_user_id == userId && t.verification_code == code))
            {
                isValid = true;
            }
        ...
        public int RemoveUser(long userMobile, long adminMobile)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    tbl_Student_user_master AdminChek = (from adm in contextsdce.tbl_Student_user_master where adm.mobile == adminMobile select adm).FirstOrDefault();
                    if (AdminChek != null)
                    {
                        //Delete UserId if Already Exist
                        tbl_Student_user_master delurp = (from del in contextsdce.tbl_Student_user_master where del.mobile == userMobile select del).FirstOrDefault();
                        if (delurp != null)
                        {
                            contextsdce.tbl_Student_user_master.Remove(delurp);
        ...
        public int GetVerificationCodeByMobileForRefundProcess(long MobNo, string emailID)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var userdetails = (from userm in contextsdce.tbl_Student_user_master where userm.mobile == MobNo && userm.email_id == emailID select userm).FirstOrDefault();
                    ...
                    if (userdetails != null)
                    {
                        ...
                        student_user_random_pass delurp = (from del in contextsdce.student_user_random_pass where del.std_user_id == userdetails.Student_user_id && del.action_type == 2 select del).FirstOrDefault();
                        if (delurp != null)
                        {
                            //contextsdce.user_random_pass.DeleteObject(delurp);
                            contextsdce.student_user_random_pass.Remove(delurp);
                            contextsdce.SaveChanges();
                        }


                        //Inserting Random password to user_random_pass Generated in verificationcode
                        student_user_random_pass urp = new student_user_random_pass();
                        urp.verification_code = sOTP;
                        urp.generated_time = DateTime.Now;
                        urp.action_type = 2;
                        urp.std_user_id = Convert.ToInt32(userdetails.Student_user_id);
                        //contextsdce.AddTouser_random_pass(urp);
                        contextsdce.student_user_random_pass.Add(urp);
                        contextsdce.SaveChanges();
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    if (userdetails != null)
                    {
                        ...
                        //Delete User Id if Already Exsist
                        student_user_random_pass delurp = (from del in contextsdce.student_user_random_pass where del.std_user_id == userdetails.Student_user_id && del.action_type == 1 select del).FirstOrDefault();
                        if (delurp != null)
                        {
                            //contextsdce.user_random_pass.DeleteObject(delurp);
                            contextsdce.student_user_random_pass.Remove(delurp);
                        ...
                        //{#var#} is the OTP to login to your LearnEngg account – LearnEngg
                        //OTP send to Mobile Number
                        strPostResponse = obj1.SendSMS(MobNo.ToString().Trim(), "" + sOTP + "  is the OTP to login to your LearnEngg account – LearnEngg", "test");
        ...
        public string GetProfileUpdateOTPCode(string MobNo, int userId)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                ...
                try
                {
                    ...
                    student_user_random_pass delurp = (from del in contextsdce.student_user_random_pass where del.std_user_id == userId && del.action_type == 1 select del).FirstOrDefault();
                    if (delurp != null)
                    {
                        contextsdce.student_user_random_pass.Remove(delurp);
                        contextsdce.SaveChanges();
        ...
        public string UpdateUserDetails(int userid, string userName, int deptId, string deptName, int year, int sem, string collegeName, int collegeId)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    var userDetails = (from user in contextsdce.tbl_Student_user_master where user.Student_user_id == userid select user).FirstOrDefault();
                    ...
                    if (userDetails != null)
                    {
                        ...
                        contextsdce.tbl_Student_user_master.AddOrUpdate(userDetails);
        ...
        public string SaveNewUserDetails(string UserName, int Department, int University, int College, int year, int Semester, string Email, long Mobile)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    tbl_Student_user_master usermaster = new tbl_Student_user_master();
                    ...
                    contextsdce.tbl_Student_user_master.Add(usermaster);
        ...
        /// <returns></returns>
        //public checkLogin CheckStudentLoginDetails(string loginID, string Password)
        //{
        ...
        //            var data = (from login in contextsdce.tbl_Student_user_master
        ...
        //                        join depart in contextsdce.department_master on login.DepartmentID equals depart.department_id
        ...
        //                             department = depart.department_code,
        //                             rollno = login.roll_number,
        //                             stud_dob = login.dob
        //                         }).ToList();
        ...
        //                                //Depa = c.department,
        //                                //dob = Convert.ToDateTime(c.stud_dob),
        //                                //roll_no = c.rollno
        ...
        //                    ////contextsdce.SaveChanges();
        //                    var user = (from login in contextsdce.tbl_Student_user_master
        //                                where (login.email_id.Trim() == loginID.Trim() || login.mobile == mobile) && login.password.Trim() == pass.Trim()
        ...
        public StudentcheckLogin CheckStudentLoginDetails(string loginID, string Password)
        {
            StudentcheckLogin chckLogin = new StudentcheckLogin();
            long mobile = 0;
            bool result = Int64.TryParse(loginID, out mobile);
            JavaScriptSerializer jsSerializer = new JavaScriptSerializer();
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var data = (from login in contextsdce.tbl_Student_user_master
                                join ur in contextsdce.user_role on login.role_id equals ur.role_id
                                join cm in contextsdce.college_master on login.collegeid equals cm.college_id into cmtemp
                                from cms in cmtemp.DefaultIfEmpty()                                 
                                join depart in contextsdce.department_master on login.DepartmentID equals depart.department_id
                                //where (login.email_id.Trim() == loginID.Trim() || login.mobile == mobile) && login.password.Trim() == pass.Trim()
                                where (login.email_id.Trim() == loginID.Trim() || login.mobile == mobile) //&& login.password.Trim() == pass.Trim()
                                select new
                                {
                                    ...
                                    rollno = login.roll_number ?? "",
                                    stud_dob = login.dob,
                                    AcademicYear = login.AcademicYear ?? "",  // AcademicYear
                                    UserRole_type = ur.role_desc ?? "",
                                    collegename = cms.college_name ?? "",
                                    CourseType = login.CourseType ?? "",
                                    login.FailedLoginAttempts,
                                     login.IsLocked,
                                    login.LockoutEndTime,
                                    login.password
                                }).ToList();
                    ...
                    var userData = (from c in data
                                    select new StudentcheckLogin
                                    {
                                        ...
                                        strdob = c.stud_dob == null ? "" : Convert.ToString(c.stud_dob),
                                        roll_no = c.rollno,
                                        //25-03-2025
                                        CollegeName = c.collegename,
                                        //CollegeName= c.collegeId.ToString(),
                                        //AcademicYear = Convert.ToInt32(c.AcademicYear),
                                        AcademicYear = c.AcademicYear,
                                        UserRole_type = c.UserRole_type,
                                        CourseType = c.CourseType,
                                        FailedLoginAttempts=c.FailedLoginAttempts == null ? 0 : c.FailedLoginAttempts,
                                        IsLocked=c.IsLocked == null ? true : c.IsLocked,
                                        LockoutEndTime=c.LockoutEndTime,
                                        Password=c.password
                                    }).ToList();
                    ...
                    if (userData.Any() == true)
                    {
                        ...
                        if (userData.FirstOrDefault().IsLocked == true && userData.FirstOrDefault().LockoutEndTime > DateTime.Now)
                        {
                            var remaining = userData.FirstOrDefault().LockoutEndTime.Value - DateTime.Now;
                            //ModelState.AddModelError("", $"Account is locked. Try again after {remaining.Minutes} minutes.");
                            userData.FirstOrDefault().message = $"Account is locked. Try again after {remaining.Minutes} minutes.";
                            chckLogin = userData.FirstOrDefault();
                            return chckLogin;
                        }
                        ...
                        //    contextsdce.SaveChanges();
                        //}
                        // Compare passwords securely (hash comparison recommended)
                        if (userData.FirstOrDefault().Password == pass.Trim()) // Replace with a secure hash check
                        {
                            ...
                            if (userData.FirstOrDefault().isFrstLogin == 1 && userData.FirstOrDefault().actveStatus == 1)
                            {
                                ...
                                if (user != null)
                                {
                                    ...
                                    // Login user
                                    FormsAuthentication.SetAuthCookie(loginID.Trim(), false);
                                }
                            }
                            else if (userData.FirstOrDefault().isFrstLogin == 2 && userData.FirstOrDefault().actveStatus == 0)
                            {
                        ...
                        {
                            int? attempt = userData.FirstOrDefault().FailedLoginAttempts + 1;
                            user.FailedLoginAttempts = attempt;
                            if (attempt >= 5)
                            {
                                user.IsLocked = true;
                            ...
                            userData.FirstOrDefault().message = "Invalid username or password.";
                        }
                    ...
                    {
                        //var userDataa = CheckCollegeLogin(loginID, Password);
                        StudentcheckLogin dt = new StudentcheckLogin();
                        dt.message = "The UserName and Password doesn't exists";
                        chckLogin = dt;
                        return chckLogin; // "No details for this user You Entered wrong Mobile or EmailId";
                    }
        ...
        public StudentcheckLogin CheckStudentSession(string loginID)
        {
            StudentcheckLogin chckLogin = new StudentcheckLogin();
            long mobile = 0;
            //bool result = Int64.TryParse(loginID, out mobile);
        ...
        public ForgetPasswordModel ForgetPassword(long mobileNo)
        {
            ...
            try
            {
                ...
                var userDetails = new StudentUserMasterAccess().GetAllUsers();
                if (userDetails.Any(t => t.mobile == mobileNo))
                {
                    ...
                    forgetModel.userID = user.Student_user_id;
                    forgetModel.message = "An OTP has been sent to the registered phone number if the user is present in our system.";
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    if (!regex.IsMatch(newPassword)) {
                        resetModel.userID = 0;
                        resetModel.message = "Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one number, and one special character.";
                        return resetModel;
                    }

                    //var userDetails = new UserMasterAccess().GetAllUsers();
                    var userDetails = (from usm in contextsdce.tbl_Student_user_master where usm.mobile == mobileNo select usm);
                    // if (userDetails.Any(t => t.mobile == mobileNo))
                    if (userDetails.Any() == true)
                    {
                        var user = userDetails.FirstOrDefault(t => t.mobile == mobileNo);
                        if (GetRandomCode(user.Student_user_id, otpCode))
                        {
                            var password = new EncryptionDecryption().encrptpwd(newPassword, true);
                            var isUpdated = new StudentUserMasterAccess().UpdatePassword(user.Student_user_id, password);
                            if (isUpdated)
                            {
                                resetModel.userID = user.Student_user_id;
        ...
        public string ChangePassword(long mobileNum, string currPasswrd)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var userpwd = (from chnagepwd in contextsdce.tbl_Student_user_master where chnagepwd.mobile == mobileNum && chnagepwd.password == curntPwd select chnagepwd);
        ...
        public string GetUserProfileImage(int UserID)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var userData = (from login in contextsdce.tbl_Student_user_master
                                    join ur in contextsdce.user_role on login.role_id equals ur.role_id
                                    where login.Student_user_id == UserID
                                    select new checkLogin
                                    {
                                        FirstName = login.user_first_name,
                                        emailId = login.email_id,
        ...
        public string UpdateContactDetails(int userid, long mobile, string emailId)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var updateContact = (from user in contextsdce.tbl_Student_user_master
                                         where user.Student_user_id == userid
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                ...
                try
                {
                    ...
                    var datasub = from a in contextsdce.user_subscribe_master
                                  join b in contextsdce.tbl_Student_user_master on a.user_id equals b.Student_user_id
        ...
        public string UpdatePackageActivation(long activate_MobileNo, long buyer_Mobileno, string activationcode)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                var User = (from um in contextsdce.tbl_Student_user_master where um.mobile == activate_MobileNo select new { um.Student_user_id, um.active_status }).First();
                if (User.Student_user_id > 0 && User.active_status > 0)
                {
                    var ude = (from a in contextsdce.purchase_activationmaster
                        ...
                        if (ude.activationstatus == 0)
                        {
                            using (var transaction = contextsdce.Database.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    ude.activated_date = DateTime.Now;
                                    ude.activated_userid = User.Student_user_id;
                                    ude.activationstatus = 2;
                                    int updatecon = contextsdce.SaveChanges();
                                    ...
                                    saveSubjct = (from usm in contextsdce.user_subscribe_master
                                                  ...
                                                  select new saveSubjeDetails
                                                  {
                                                      ...
                                                      activDurDate = DateTime.Now,
                                                      released_On = sm.released_on,
                                                      user_ID = (int)User.Student_user_id,
                                                      is_updated_to_client = 1,
                                                      LastmodifiedOn = DateTime.Now,
                                                      LastClientUpdatedOn = DateTime.Now,
                                                      LastSubunitModifiedOn = DateTime.Now,
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    saveSubjct = (from sm in contextsdce.subject_master
                                  join dsm in contextsdce.department_subject_mapping on sm.subject_id equals dsm.subject_id
                                  join um in contextsdce.tbl_Student_user_master on sm.UniversityID equals um.univ_id
                                  join dm in contextsdce.department_master on dsm.department_id equals dm.department_id
                                  join sbum in contextsdce.subject_unit_master on sm.subject_id equals sbum.subject_id
                                  join roltyp in contextsdce.user_role on um.role_id equals roltyp.role_id
                                  where um.Student_user_id == userid && sm.active_status == 1 && sbum.active_status == 1
        ...
        public string OTP_CodeVerification(int userId, string otpCode)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    var codeVerification = (from randomcode in contextsdce.student_user_random_pass
                                            where randomcode.std_user_id == userId && randomcode.verification_code == otpCode
                                            select randomcode);

                    if (codeVerification.Any() == true)
                    {
                        var userUpdate = (from user_update in contextsdce.tbl_Student_user_master
                                          where user_update.Student_user_id == userId
        ...
        public List<UserDetails> GetUserDetails(int studentId)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    EncryptionDecryption encrypt = new EncryptionDecryption();
                    userDetails = (from um in contextsdce.tbl_Student_user_master
                                   ...
                                   where um.Student_user_id == studentId
                                   select new UserDetails
                                   {
                                       userid = um.Student_user_id,
        ...
        public string UpdateUserProfileImage(Int64 UserID, string ImageName)
        {
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    var userImg = (from um in contextsdce.tbl_Student_user_master where um.Student_user_id == UserID select um).FirstOrDefault();
        ...
        public UseageDashBoardViewModel UserUsageDashBoard(int SubjectType, int UserId)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    // ITIwise Usage
                    var CollegewiseUsage = (from um in contextsdce.tbl_Student_user_master
                                            join usrh in contextsdce.user_subject_read_history on um.Student_user_id equals usrh.user_id
                                            ...
                                            where cm.active_status == 1 && sub.having_questionpaper == SubjectType && um.Student_user_id == UserId
                    ...
                    // Montwise Usage
                    var MonthwiseUsage = (from um in contextsdce.tbl_Student_user_master
                                          join usrh in contextsdce.user_subject_read_history on um.Student_user_id equals usrh.user_id
                                          join sub in contextsdce.subject_master on usrh.subject_id equals sub.subject_id
                                          //     join csm in contextsdce.college_subject_mapping on um.collegeid equals csm.college_id
                                          //     join cm in contextsdce.college_master on um.collegeid equals cm.college_id
                                          where sub.having_questionpaper == SubjectType && um.Student_user_id == UserId
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    int RegisteredUserCount = (from cm in contextsdce.college_master
                                               join um in contextsdce.tbl_Student_user_master on cm.college_id equals um.collegeid
                                               where um.active_status == 1 && cm.active_status == 1 && um.role_id < 3
                                               select um.Student_user_id).Distinct().Count();
        ...
        public UseageDashBoardViewModel CollegeUsageDashBoard(int SubjectType, int CollegeId)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    var TradewiseUsage = (from sub in contextsdce.subject_master
                                          join usrh in contextsdce.user_subject_read_history on sub.subject_id equals usrh.subject_id
                                          join um in contextsdce.tbl_Student_user_master on usrh.user_id equals um.Student_user_id
                                          join dsm in contextsdce.department_subject_mapping on sub.subject_id equals dsm.subject_id
                                          join dm in contextsdce.department_master on dsm.department_id equals dm.department_id
                                          where sub.active_status == 1 // && sub.having_questionpaper == SubjectType
        ...
        #region subject time capture
        public Student_subject_read_history StudentSubjectTimeSave(int userid, int DepartmentID, string fileName)
        {
            try
            {
                using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
                {
                    ...
                    Student_subject_read_history Student_Subject_Read_Histories = new Student_subject_read_history();
                    //var subject = contextsdce.user_subject_read_history.ToList();
                    //var subjectmaster = contextsdce.subject_master.Where(x => x.subject_code == fileName).ToList();
                    var subjectID = contextsdce.subject_master.Where(x => x.subject_code == fileName).Select(y=>y.subject_id).FirstOrDefault();
                    //var currentuser = contextsdce.tbl_Student_user_master.Where(s => s.Student_user_id == userid).ToList();
                    //Save this data once student get into file
                    //foreach (var item in subjectmaster)
                    //{

                    //var latestReadHistory = contextsdce.user_subject_read_history.Select(x=>x.max(user_read_history_id)).OrderByDescending(s => s.user_read_history_id)
                    //   .Take(1).ToList();

                    Student_Subject_Read_Histories = new Student_subject_read_history();
                    Student_Subject_Read_Histories.Student_id = userid;
                    Student_Subject_Read_Histories.department_id = DepartmentID;
                    Student_Subject_Read_Histories.subject_id = Convert.ToInt32(subjectID);
                    Student_Subject_Read_Histories.total_hours = 0;
                    Student_Subject_Read_Histories.last_read_on = DateTime.Now;  //use  Parameter

                    contextsdce.Student_subject_read_history.Add(Student_Subject_Read_Histories);
                    contextsdce.SaveChanges();

                    //}
                    //var latestReadHistory = contextsdce.user_subject_read_history.Where(y=>y.user_id == userid)
                    //    .OrderByDescending(s => s.user_read_history_id).Select(x => x.user_read_history_id)
                    //    .Take(1);


                    //     var latestReadHistory = contextsdce.user_subject_read_history.Where(y => y.user_id == userid)
                    //.OrderByDescending(s => s.user_read_history_id)
                    //.Take(1).Select(x => { x.user_read_history_id, x.subject_id});

                    var latestReadHistory = contextsdce.Student_subject_read_history
    .Where(y => y.Student_id == userid)
    .OrderByDescending(s => s.Student_read_history_id)
    .Select(x => new { x.Student_read_history_id, x.subject_id })
    .FirstOrDefault();

                    //foreach (var item in latestReadHistory)
                    //{
                    HttpContext.Current.Session["last_read_on"] = latestReadHistory.Student_read_history_id;
                        HttpContext.Current.Session["subject_id"] = latestReadHistory.subject_id;
                        //HttpContext.Current.Session["StudebtReadingStartTime"] = DateTime.Now; //use  Parameter
                    //}
                    //user_Subject_Read_Histories.department_id = DepartmentID;
                    return Student_Subject_Read_Histories;
                }
            }
            catch (Exception ex)
            {
        ...
        #endregion
        #region end session subject time capture
        public Student_subject_read_history StudentSubjectEndTimeSave(int userid, int subject_id, int history_ID, int DepartmentID)
        {
            try
            {
                using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
                {
                    //update
                    var recordToUpdate = contextsdce.Student_subject_read_history
                                         .FirstOrDefault(s => s.Student_read_history_id == history_ID);
                    if (recordToUpdate != null)
                    {
                        //////start time
                        ////DateTime inTime = DateTime.Parse(recordToUpdate.last_read_on.ToString());  // 9:30 AM                                                                       
                        ////string timeOnly = inTime.ToString("HH:mm");
                        ////TimeSpan timeSpan = TimeSpan.Parse(timeOnly);
                        ////int totalMinutes_IN = (timeSpan.Hours * 60) + timeSpan.Minutes;

                        //////end time
                        ////string outtime = DateTime.Now.ToString();
                        ////DateTime outTime = DateTime.Parse(outtime);
                        ////string outTimeonly = outTime.ToString("HH:mm");
                        ////TimeSpan timeSpanOut = TimeSpan.Parse(outTimeonly);
                        ////int totalMinutes_Out = (timeSpanOut.Hours * 60) + timeSpanOut.Minutes;
                        ////int total_hours = totalMinutes_Out - totalMinutes_IN;

                        //string crntTime = DateTime.Now.Hour.ToString();
                        //int total_hours =Convert.ToInt32(crntTime) -Convert.ToInt32(totaltiming);


                        DateTime? a = DateTime.Now;
                        DateTime? b = recordToUpdate.last_read_on;
                        //  TimeSpan duration = a.Value - b.Value;
                        var MinutesDiff = a.Value.Subtract(b.Value).TotalSeconds;
                       int IntMinutesDiff = Convert.ToInt32(MinutesDiff);





                        recordToUpdate.total_hours = IntMinutesDiff;
                        contextsdce.SaveChanges();  // Save changes to the database
                    }
