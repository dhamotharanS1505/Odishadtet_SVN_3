    ...
    public class AdminService : IAdminService
    {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    ...
                    if (userdetails != null)
                    {
                        ...
                        {
                            Log.WriteLogMessage(PageName, "NewRegistration", "GetVerificationCodeByMobile email sending", ex1.Message, "error");
                        }
        ...
        public List<CollegeModel> GetCollegeAdministrators(int pcollege_group_id)
        {
            ...
            using (learnengg_payment_portal_entities contextsdce = new learnengg_payment_portal_entities())
            {
                try
                {
                    //CM = (contextsdce.college_master.ToList().OrderBy(x => x.college_code)).Distinct().ToList().Select(x=> new CollegeModel
                    //{
                    //    college_code= x.college_code,
                    //    college_name = x.college_name,
                    //    college_email = x.college_email,
                    //    college_mobile = x.college_mobile,
                    //    total_license = x.total_license,
                    //    active_status = x.active_status
                    //}).ToList<CollegeModel>();


                    CM = (contextsdce.college_master
                        .Join(contextsdce.college_group_map,cm=>cm.college_id,cgm=>cgm.college_id, (cm,cgm) => new { cm, cgm})
                        .Join(contextsdce.college_group_master,  cgm => cgm.cgm.college_group_id, cg => cg.college_group_id, (cgm, cg) => new { cgm, cg })
                        .Where (cg=>cg.cg.college_group_id == pcollege_group_id)
                        .ToList().OrderBy(x => x.cgm.cm.college_code)).Distinct().ToList()
                        .Select(x => new CollegeModel
                    {
                        college_code = x.cgm.cm.college_code,
                        college_name = x.cgm.cm.college_name,
                        college_email = x.cgm.cm.college_email,
                        college_mobile = x.cgm.cm.college_mobile,
                        total_license = x.cgm.cm.total_license,
                        active_status = x.cgm.cm.active_status
                    }).ToList<CollegeModel>();
                ...
                {
                    Log.WriteLogMessage(PageName, "AdminService", "GetCollegeAdministrators", ex.Message, "error");
                    throw;
                }
                finally
                {
        ...
        /// </summary>
        /// <returns></returns>
        public string SampleMail(string pEmailId)
        {
            string result = "Fails";
            var template = "<div></div>";
            try
            {
                MailHelper.SendMail(pEmailId, "One Time PassWord", template);
                result = "Success";
            }
            catch (Exception)
            {
